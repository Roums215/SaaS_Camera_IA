...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host ""
        Write-Host "[OK] All services restarted successfully" -ForegroundColor Green
        ...
        Write-Host ""
        Write-Host "[!] Some services may not have restarted properly" -ForegroundColor Yellow
    }
} catch {
    Write-Host ""
    Write-Host "[X] Error restarting services: $_" -ForegroundColor Red
}
